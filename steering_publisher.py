...
class LaneDetection:
    ...
    def __init__(self):
        ...
        self.birdeye_pub = rospy.Publisher("/birdeye", Image, queue_size=5)
        self.frame_pub = rospy.Publisher("/frame", Image, queue_size=5)
        rospy.spin()
    ...
    def convertIng(self, img):
        ...
        for i in self.slidng_list_left:
            cv2.circle(bird_eye_image, i, 7, (0, 0, 255), -1)
        ...
        self.birdeye_pub.publish(self.cvbridge.cv2_to_imgmsg(bird_eye_image, "rgb8"))
        self.frame_pub.publish(self.cvbridge.cv2_to_imgmsg(frame_steer, "rgb8"))
    ...
    def steering(self, frame, slidng_list_left, sliding_list_right):
        ...
        for i in range(0, len(left_diff_arr)): 
            #the value 20 can be modified
            if (left_diff_arr[i] > (left_avg +20)) or (left_diff_arr[i] < (left_avg - 20)): 
        ...
        for i in range(0, len(right_diff_arr)): 
            #the value 20 can be modified
            if (right_diff_arr[i] > (right_avg +20)) or (right_diff_arr[i] < (right_avg - 20)): 
